...
Tetrion.prototype.update = function(){
    ...
    tetrion.activeTetromino.moveDown();
    tetrion.draw();
};
...
Tetrion.prototype.draw = function(){
    ...
    for (var y = 0; y < this.rows; y++) {
        for (var x = 0; x < this.columns; x++) {
            if(this.matrix[y][x] == 1){
                ctxt.fillStyle = this.colorMatrix[y][x];
    ...
    this.activeTetromino.draw(this.activeTetromino.position,this.activeTetromino.color);

    for (var i = 0; i < this.tetrominoBuffer.length; i++) {
        this.tetrominoBuffer[i].draw(new Vector(11, 1 + i * 3), this.tetrominoBuffer[i].color)
    }
...
Tetrion.prototype.placeTetromino = function(){
